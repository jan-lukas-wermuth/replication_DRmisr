...
save("F_(0_0)", file = "/Workspace/YRBS_DR_nomis/DistributionRegression_20172019_rec/F_(0_0).RData")
save("F_(1_0)", file = "/Workspace/YRBS_DR_nomis/DistributionRegression_20172019_rec/F_(1_0).RData")
save("F_(0_1)", file = "/Workspace/YRBS_DR_nomis/DistributionRegression_20172019_rec/F_(0_1).RData")
save("F_(1_1)", file = "/Workspace/YRBS_DR_nomis/DistributionRegression_20172019_rec/F_(1_1).RData")
...
"F_(0_0)_b" <- function(j, B, drug){
  ...
  res <- foreach(i = 1:B, .combine = 'c', .export = paste("yrbs_data_combined_20172019_norecreational", drug, sep = "_")) %dopar% {
    ...
    x_length <- ncol(X_mat)
    z_length <- ncol(Z_mat)
    formula_mar <- paste(paste(drug, j, sep = ""), "~", "year + median_income + sex + unemployment_rate + mar_prices_medium")
    formula_mar_life <- paste(paste(drug, j, sep = ""), "~", "year + median_income + sex + unemployment_rate + mar_prices_medium")
    formula_cig <- paste(paste(drug, j, sep = ""), "~", "year + median_income + sex + unemployment_rate + tax_percent")
...
# Compute critical value: Counterfactual Effect (0_1) - (0_0)
