# Title:      Function Collection for the Script MisreportingY.R
...
#             probabilities for Y while allowing for misreporting.
...
"F_(0|0)" <- function(y, drug){
  ...
  x_length <- ncol(X_mat)
  z_length <- ncol(Z_mat)
  theta <- get(paste(drug, "gjrm_norecreational", y, sep = "_"))$theta
...
"F_(1|0)" <- function(y, drug){
  ...
  x_length <- ncol(X_mat)
  z_length <- ncol(Z_mat)
  theta <- get(paste(drug, "gjrm_recreational", y, sep = "_"))$theta
